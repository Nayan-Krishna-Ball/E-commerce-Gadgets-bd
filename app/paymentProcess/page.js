...
import PaymentProcessHeader from "@/components/payment/paymentProcessHeader";
import { getCartByUserId, getProductByProductId } from "@/queries";
...
export default async function PaymentProcessPage({ searchParams }) {
  ...
  //Get Product by product id from seacrhParams
  const { mode, productId, qty } = searchParams;
  let singleProduct = null;

  if (mode === "buyNow" && productId) {
    singleProduct = await getProductByProductId(productId);
  }

  //check item type here
  const items =
    mode === "buyNow"
      ? [{ product: singleProduct, quantity: qty }]
      : allCartItems?.items || [];
  ...
  return (
    <div className="bg-amazon-background text-amazon-text flex flex-col min-h-screen">
      <PaymentProcessHeader allCartProductLength={totalItemsLength} />
      <PaymentProcess
        user={user}
